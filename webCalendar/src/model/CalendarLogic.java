...
public class CalendarLogic {
    ...
    public void buildList() {
        ...
        //---- prev adapter ----
        if (month == 1) {
            prevList = buildCalendar(year - 1, month + 11, prevList);
        } else {
            prevList = buildCalendar(year, month - 1, prevList);
        }

        //---- next adapter ----
        if (month == 12) {
            nextList = buildCalendar(year + 1, month - 11, nextList);
        } else {
            nextList = buildCalendar(year, month + 1, nextList);
        }
