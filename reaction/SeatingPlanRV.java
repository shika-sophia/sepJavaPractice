...
【考察２】追記 2020-10-20
上記の修正前に、なぜ結果表が出たのかというと
continue loop;によって、ラベル「loop:」のある do{}のほうに飛んで再シャッフルすると思っていたが、
下の終了条件式 while(checked);のほうに飛ぶようだ。

つまり、旧座席との かぶりがあった場合、初期値に設定した checked = falseのまま
while(checked)に入り、そのままループを抜けて、結果表を表示。
結果表が出たケースは、必ず旧座席との かぶりがあるというダメダメな結果に・・
(結果表が出て喜んでる場合じゃなかった)

while(!checked);に修正し、checked = falseのときは再シャッフル、
かぶりチェックの for文を最後まで抜けると checked = true;が代入され、do{}while();ループを抜けるという
正しい動作になりました。
*/