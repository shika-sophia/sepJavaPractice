/**
 * @title javaSilver / SE11Black / Chapter06 / 第６章 インスタンスとメソッド
 ...
 * @date 2021-01-13 / 16:14-16:51 (37分)
 * @correctRate ① 16 / 30 = 53.3％
 * @correctRate ②80.00 ％ ( 〇24問 / 全30問 )
 */
...
〇 4: B
× 5: D => 〇: C
    -> staticフィールドは static main()から参照可。
    -> 「インスタンス変数.field」は、staticフィールドの場合、「クラス名.field」にコンパイラが変換。
    -> static fieldは、どのインスタンスからも共通の値
〇 6: D,E
〇 7: E
〇 8: D
〇 9: A, B
...
〇 16: B, D
× 17: D => 〇: A
    -> calc()の定義はオーバーロードの条件を満たす。
    -> calc(2, 3)も int -> dobuleに型変換され問題ないが
       それに適合する calc()は２つあるためコンパイルエラー
〇 18: E
〇 19: D
× 20: D => 〇: A
   -> 戻り値を記述したコンストラクタは通常のメソッドと解釈される。
      (コンパイルエラーにはならない)
〇 21: B
〇 22: A
× 23: A => 〇: D
   -> void Sample(){ }は通常のメソッド。
   -> Sample(String){ }があるため、デフォルトSample()が生成されずコンパイルエラー。
〇 24: B
〇 25: E
× 26: B => 〇: D
   -> numは無記のためパッケージprivate。
   -> 継承関係にあっても、別パッケージのためアクセス不可。
〇 27: C
   -> 継承関係にあるインスタンスは２つ以上のインスタンスの特徴を併せ持つ。
   -> Bookの定義はpackage otherでのprotected。Main.main()は継承関係になく別メソッドのためアクセス不可。
〇 28: D
〇 29: A
〇 30: B
